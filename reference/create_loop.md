...
- parent:
  ...
  manually call
  [`run_now()`](https://later.r-lib.org/reference/run_now.md) on this
  loop. If `NULL`, then this loop will not have a parent event loop that
  automatically runs it; the only way to run this loop will be by
  calling [`run_now()`](https://later.r-lib.org/reference/run_now.md) on
  this loop.
...
[`later()`](https://later.r-lib.org/reference/later.md) or
[`run_now()`](https://later.r-lib.org/reference/run_now.md) will use the
