...
- Fixed [\#215](https://github.com/r-lib/later/issues/215): The
  `autorun` argument of
  [`create_loop()`](https://later.r-lib.org/reference/create_loop.md),
...
- Adds [`later_fd()`](https://later.r-lib.org/reference/later_fd.md)
...
- Added private event loops: these are event loops that can be run
  ...
  [`later()`](https://later.r-lib.org/reference/later.md), and you want
  to call [`run_now()`](https://later.r-lib.org/reference/run_now.md)
  block and wait for those callbacks to execute before continuing.
  Without private event loops, if you call
  [`run_now()`](https://later.r-lib.org/reference/run_now.md) to wait
  until a particular callback has finished, you might inadvertantly run
  other callbacks that were scheduled by other code. With private event
  loops, you can create a private loop, schedule a callback on it, then
  call [`run_now()`](https://later.r-lib.org/reference/run_now.md) on
...
- Added `all` argument to
  [`run_now()`](https://later.r-lib.org/reference/run_now.md); defaults
  to `TRUE`, but if set to `FALSE`, then `run_now` will run at most one
  later operation before returning. PR
...
- Fixed issue [\#39](https://github.com/r-lib/later/issues/39): Calling
  the C++ function
  [`later::later()`](https://later.r-lib.org/reference/later.md) from a
  different thread could cause an R GC event to occur on that thread,
  leading to memory corruption. PR
...
- Fixed issues [\#25](https://github.com/r-lib/later/issues/25),
  ...
  [`run_now()`](https://later.r-lib.org/reference/run_now.md)), then
...
- The [`run_now()`](https://later.r-lib.org/reference/run_now.md)
  ...
  [`run_now()`](https://later.r-lib.org/reference/run_now.md) will
...
- The [`run_now()`](https://later.r-lib.org/reference/run_now.md)
  ...
  [`run_now()`](https://later.r-lib.org/reference/run_now.md) would
  never return. This release changes that behavior so that a timestamp
  is taken as
  [`run_now()`](https://later.r-lib.org/reference/run_now.md) begins
...
- Added
  [`next_op_secs()`](https://later.r-lib.org/reference/next_op_secs.md)
...
- Add [`loop_empty()`](https://later.r-lib.org/reference/loop_empty.md)
